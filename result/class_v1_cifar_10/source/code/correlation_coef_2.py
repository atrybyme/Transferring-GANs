...
############################################################################################################################################
clas_real_datas = tf.placeholder(
    tf.float32, [None, cfg.iDimsC, 32, 32], name='real_datas')
clas_real_labels = tf.placeholder(tf.int32, shape=[None])
...
with tf.variable_scope('classifier', tf.AUTO_REUSE):

    h0 = ckas_real_datas
    h1 = noise(h0, 0.1, bAdd=False, bMulti=False)

    h2 = conv2d(h1, iFilterDimsD * 1, ksize=3,
...
with tf.variable_scope('discriminator', tf.AUTO_REUSE):
    ...
    d0 = real_datas
    d1 = noise(d0, 0.1, bAdd=False, bMulti=False)

    d2 = conv2d(d1, iFilterDimsD * 1, ksize=3,
                stride=1, name='conv32')  # 32x32
    d3 = tf.nn.leaky_relu(d2, name='d3')
    d4 = dropout(d3, cfg.fDropRate)

    d5 = conv2d(d4, iFilterDimsD * 2, ksize=3, stride=2,
                name='conv32_16')  # 32x32 --> 16x16
    d6 = batch_norm(d5, name='bn16')
    d7 = tf.nn.leaky_relu(d6, name='d7')
    d8 = dropout(d7, cfg.fDropRate)

    d9 = conv2d(d8, iFilterDimsD * 4, ksize=3, stride=2,
                name='conv16_8')  # 16x16 --> 8x8
    d10 = batch_norm(d9, name='bn8')
    d11 = tf.nn.leaky_relu(d10, name='d11')
    d12 = dropout(d11, cfg.fDropRate)

    d13 = conv2d(d12, iFilterDimsD * 8, ksize=3,
                 stride=2, name='conv8_4')  # 8x8 --> 4x4
    d14 = batch_norm(d13, name='bn4')
    d15 = tf.nn.leaky_relu(d14, name='d15')
    d16 = dropout(d15, cfg.fDropRate)

    d17 = avgpool(d16, d16.get_shape().as_list()[
        2], d16.get_shape().as_list()[3])
    d18 = tf.contrib.layers.flatten(d17)
    d19 = dropout(d18, cfg.fDropRate)

    d20 = linear(d19, num_logits+1)
...
real_logits = d20
clas_real_logits = h20

dis_total_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=real_logits, labels=real_labels))
clas_total_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=clas_real_logits, labels=clas_real_labels))

prediction = tf.cast(tf.argmax(real_logits, 1), tf.int32)
equality = tf.equal(prediction, real_labels)
accuracy = tf.reduce_mean(tf.cast(equality, tf.float32))

clas_prediction = tf.cast(tf.argmax(clas_real_logits, 1), tf.int32)
clas_equality = tf.equal(clas_prediction, clas_real_labels)
clas_accuracy = tf.reduce_mean(tf.cast(clas_equality, tf.float32))

tot_vars = tf.trainable_variables()
dis_vars = [var for var in tot_vars if 'discriminator' in var.name]
clas_vars = [var for var in tot_vars if 'classifier' in var.name]




weights_to_new_layer_mapping = {}


# ##############################
# checkpoint_dir = SOURCE_DIR + 'result/class_v5_cifar10/checkpoint'
# ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
# v = tf.train.list_variables(c)
# for i in (v):
#     old_name = i[0]
#     new_name = old_name.replace('discriminator', 'classifier')
#     new_name = new_name+':0'
#     new = tf.get_default_graph().get_tensor_by_name(new_name)
#     weights_to_new_layer_mapping.update({old_name: new})
# ################################
for i in clas_vars:
    old_name = 'discriminator' +i.name.replace('classifier','')
    old_name = old_name.replace(':0','')
    new_name  = i
    print(i.name)
    weights_to_new_layer_mapping.update({old_name:new_name})
print(weights_to_new_layer_mapping)
...
clas_optimizer = tf.train.AdamOptimizer(
    learning_rate=lr, beta1=cfg.fBeta1, beta2=cfg.fBeta2, epsilon=cfg.fEpsilon)
clas_gradient_values = dis_optimizer.compute_gradients(
    dis_total_loss, var_list=clas_vars)
clas_optimize_ops = dis_optimizer.apply_gradients(
    dis_gradient_values, global_step=global_step)
print(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, 'discriminator'))
saver = tf.train.Saver(var_list=weights_to_new_layer_mapping, max_to_keep=1000)
...
for chpnt in check_pnts:
    #print(tf.train.list_variables(chpnt))

    saver.restore(sess, chpnt)
    print("Loading ", chpnt, " Complete.")

    out_conv_1, out_conv_2, out_conv_3, out_conv_4 = sess.run(
        [h3, h7, h11, h15], feed_dict={real_datas: checking_data})
...
for i in range(num_ckpts):
    ...
    for j in range(num_conv):
        ...
        for k in range(num_conv):
            ...
            # Compute CCA
            ccas = fourier_ccas(out_train_max, out_train_curr,
                                return_coefs=True, compute_dirns=True)

            # Find number of dirs = min(num_neurons1, num_neurons2)
            num_direc = min(ccas['cca_coef1'][0].shape[0],
                            ccas['cca_coef2'][0].shape[0])

            # Find mean corr coeff
            corr_coeffs[i][j][k] = np.mean(np.array(
                [ccas['cca_coef1'][i][:num_direc] for i in range(ccas['cca_coef1'].values.shape[0])]))
            print('{},{},{} : Corr Coeff {}'.format(
                i, j, k, corr_coeffs[i, j, k]))
