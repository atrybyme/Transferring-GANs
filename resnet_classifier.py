...
tf.app.flags.DEFINE_string("sResultTag", "class_v1",
...
def basic_block(input, filters, ksize, stride, name=None):
    ...
    conv_residual = conv2d(conv_residual, filters, ksize=ksize,stride=stride, name=name + 'conv1')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = batch_norm(conv_residual, name=name+'conv_batch_norm2')
    conv_residual = conv2d(conv_residual, filters, ksize=ksize,stride=stride, name=name + 'conv2')
    conv_residual = tf.layers.average_pooling2d(conv_residual, 2, 2, data_format='channels_first', name=name+'conv_downsample')
    ##identity
    identity = conv2d(h0, filters, ksize=1, stride=1, name=name+'identity')
    identity = tf.layers.average_pooling2d(identity, 2, 2, data_format='channels_first', name=name+'identity_downsample')
...
def discriminator_dcgan(input, num_logits):
    ...
    with tf.variable_scope('discriminator', tf.AUTO_REUSE):
        ...
        # print(h0.get_shape())
        # h0 = conv2d(h0, iFilterDimsD * 1, ksize=3,
        ...
        # h0 = conv2d(h0, iFilterDimsD * 2, ksize=3, stride=2,
        ...
        # h0 = conv2d(h0, iFilterDimsD * 4, ksize=3, stride=2,
        ...
        # print(h0.get_shape())
        # h0 = conv2d(h0, iFilterDimsD * 8, ksize=3,
        ...
        # print(h0.get_shape())
        # h0 = conv2d(h0, iFilterDimsD * 16, ksize=3,
        #             stride=2, name='conv8__4')  # 8x8 --> 4x4
        # print(h0.get_shape())
        # h0 = batch_norm(h0, name='bn_4')
        # print(h0.get_shape())
        # h0 = activate(h0, cfg.oAct)
        # print(h0.get_shape())
        # h0 = dropout(h0, cfg.fDropRate)
...
def load_dataset(dataset_name):
    return load_cifar10() if dataset_name is "cifar10" else load_imagenet()
...
##checking
alfa = tf.nn.sparse_softmax_cross_entropy_with_logits(
    logits=real_logits, labels=real_labels)
...
while iter <= cfg.iMaxIter:
    ...
    r_lab, r_log, _alfa, _, _dis_total_loss, _lr, _acc = sess.run(
        [real_labels, real_logits, alfa, dis_optimize_ops,
            dis_total_loss, lr, accuracy],
