...
def discriminator_dcgan(input, num_logits):
    ...
    with tf.variable_scope('discriminator', tf.AUTO_REUSE):
        ...
        h0 = basic_block(input, iFilterDimsD, ksize=3,
                         stride=1, name='basic_block')
        h0 = residual_block(h0, iFilterDimsD*2, ksize=3,
                            stride=1, name='res_block1')
        h0 = residual_block(h0, iFilterDimsD*4, ksize=3,
                            stride=1, name='res_block2')
        h0 = residual_block(h0, iFilterDimsD*8, ksize=3,
                            stride=1, name='res_block3')
        h0 = residual_block(h0, iFilterDimsD*16, ksize=3,
                            stride=1, name='res_block4')
        print(h0.get_shape())
...
def generator_dcgan(num_sample, z=None):
    ...
    with tf.variable_scope('generator', tf.AUTO_REUSE):
        ...
        h0 = residual_block_generator(
            h0, iFilterDimsG, ksize=3, stride=2, name='generator_block4_32x32')
...
if cfg.sDataSet == 'cifar10' or cfg.sDataSet == 'mnist' or cfg.sDataSet == 'imagenet':
    ...
    real_datas = tf.placeholder(
        tf.float32, [None, cfg.iDimsC, 64, 64], name='real_datas')
...
if cfg.bAMGAN:
    ...
    dis_fake_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=fake_logits, labels=tf.ones_like(real_labels)*1000))

    dis_gan_loss = dis_fake_loss + dis_real_loss
    gen_gan_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=fake_logits, labels=tf.stop_gradient(tf.to_int32(tf.arg_max(fake_logits[:, :1000], 1)))))
...
if cfg.bLoadPre:
    ...
    load_dis_vars = [
        v for v in load_dis_vars if 'discriminator/conv32/' in v.name]
