...
tf.app.flags.DEFINE_string("sResultTag", "gan_v0",
                           "your tag for each test case")

tf.app.flags.DEFINE_boolean("bLoadPre", False, "pretrained model to be loaded")
tf.app.flags.DEFINE_string("sPreTag", "", "pretrained model tag")
...
tf.app.flags.DEFINE_boolean("bAMGAN",False, "")

tf.app.flags.DEFINE_integer("iMaxIter", 50000, "")
tf.app.flags.DEFINE_integer("iBatchSize", 50, "")
...
def basic_block(input, filters, ksize, stride, name=None):
    h0 = input
    conv_residual = batch_norm(h0, name=name+'conv_batch_norm1')
    conv_residual = conv2d(conv_residual, filters,
                           ksize=ksize, stride=stride, name=name + 'conv1')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = batch_norm(conv_residual, name=name+'conv_batch_norm2')
    conv_residual = conv2d(conv_residual, filters,
                           ksize=ksize, stride=stride, name=name + 'conv2')
    conv_residual = tf.layers.average_pooling2d(
        conv_residual, 2, 2, data_format='channels_first', name=name+'conv_downsample')
    ##identity
    identity = conv2d(h0, filters, ksize=1, stride=1, name=name+'identity')
    identity = tf.layers.average_pooling2d(
        identity, 2, 2, data_format='channels_first', name=name+'identity_downsample')

    return conv_residual + identity


def residual_block(input, filters, ksize, stride, name=None):
    h0 = input
    input_filter = (h0.get_shape())[1]
    conv_residual = batch_norm(h0, name=name+'conv_batch_norm1')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = conv2d(conv_residual, input_filter, ksize=ksize,
                           stride=stride, name=name + 'conv1')
    conv_residual = batch_norm(conv_residual, name=name+'conv_batch_norm2')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = conv2d(conv_residual, filters, ksize=ksize,
                           stride=stride, name=name + 'conv2')
    conv_residual = tf.layers.average_pooling2d(
        conv_residual, 2, 2, data_format='channels_first', name=name+'conv_downsample')
    identity = conv2d(h0, filters, ksize=1, stride=1, name=name+'identity')
    identity = tf.layers.average_pooling2d(
        identity, 2, 2, data_format='channels_first', name=name+'identity_downsample')
    return identity + conv_residual
...
def discriminator_dcgan(input, num_logits):
    ...
    with tf.variable_scope('discriminator', tf.AUTO_REUSE):
        ...
        h0 = basic_block(input, iFilterDimsD, ksize=3,stride=1, name='basic_block')
        h0 = residual_block(h0, iFilterDimsD*2, ksize=3,stride=1, name='res_block1')
        h0 = residual_block(h0, iFilterDimsD*4, ksize=3,stride=1, name='res_block2')
        h0 = residual_block(h0, iFilterDimsD*8, ksize=3,stride=1, name='res_block3')
        #h0 = residual_block(h0, iFilterDimsD*16, ksize=3,stride=1, name='res_block4')
        print(h0.get_shape())
        h0 = avgpool(h0, h0.get_shape().as_list()[
                     2], h0.get_shape().as_list()[3])
        print(h0.get_shape())
...
def residual_block_generator(input, filters, ksize, stride, name=None):
    h0 = input
    conv_residual = batch_norm(h0, name=name+'conv_batch_norm1')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = deconv2d(conv_residual, filters, ksize=ksize,
                             stride=stride, name=name + 'deconv')
    conv_residual = batch_norm(conv_residual, name=name+'conv_batch_norm2')
    conv_residual = activate(conv_residual, cfg.oAct)
    conv_residual = conv2d(conv_residual, filters, ksize=ksize,
                           stride=1, name=name + 'conv')
    identity = deconv2d(h0, filters, ksize=ksize,
                        stride=stride, name=name+'identity')
...
def generator_dcgan(num_sample, z=None):
    ...
    with tf.variable_scope('generator', tf.AUTO_REUSE):
        ...
        h0 = linear(h0, 4 * 4 * (iFilterDimsG * 16))  # linear 4x4
        h0 = tf.reshape(h0, [-1, iFilterDimsG * 16, 4, 4])  # reshape 4x4

        h0 = residual_block_generator(
            h0, iFilterDimsG * 8, ksize=3, stride=2, name='generator_block1_4x4')
        h0 = residual_block_generator(
            h0, iFilterDimsG * 4, ksize=3, stride=2, name='generator_block2_8x8')
        h0 = residual_block_generator(
            h0, iFilterDimsG * 2, ksize=3, stride=2, name='generator_block3_16x16')
        #h0 = residual_block_generator(h0, iFilterDimsG, ksize=3, stride=2, name='generator_block4_32x32')
        h0 = batch_norm(h0, name='bn_64x64')
        h0 = activate(h0, cfg.oAct)
        h0 = conv2d(h0, cfg.iDimsC, ksize=3, stride=1, name='conv_64x64')
...
def plot_generated_toy_data(X_gen, X_real, gen_iter, dir):
    ...
    plt.figure()
    plt.scatter(X_gen[:5000, 0],  X_gen[:5000, 1], s=0.5,
                color="red", marker="o", label='fake_data')
    plt.scatter(X_real[:5000, 0], X_real[:5000, 1], s=0.5,
                color="blue", marker="o", label='real_data')
    plt.legend()
...
def get_score(samples):
    ...
    if ref_icp_activations is None:
        ...
        ref_icp_preds, ref_icp_activations = icp_model.get_preds(
            data_X.transpose(0, 2, 3, 1))
        logger.log('\nref_icp_score: %.3f\n' %
    ...
    icp_preds, icp_activcations = icp_model.get_preds(
        samples.transpose(0, 2, 3, 1))
...
if cfg.sDataSet == 'cifar10' or cfg.sDataSet == 'mnist' or cfg.sDataSet == 'imagenet':
...
if cfg.bAMGAN:
    ...
    dis_real_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=real_logits, labels=real_labels))
    dis_fake_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=fake_logits, labels=tf.ones_like(real_labels)*10))

    dis_gan_loss = dis_fake_loss + dis_real_loss
    gen_gan_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=fake_logits, labels=tf.stop_gradient(tf.to_int32(tf.arg_max(fake_logits[:, :10], 1)))))
    ...
    dis_gan_loss = dis_fake_loss + dis_real_loss
    gen_gan_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
...
if cfg.oDecay == 'linear':
    ...
    lrD = tf.train.exponential_decay(
        cfg.fLrIniD, global_step, cfg.iMaxIter // 10, 0.5, True)
else:
    lrD = cfg.fLrIniD
...
if cfg.oDecay == 'linear':
    ...
    lrG = tf.train.exponential_decay(
        cfg.fLrIniG, global_step, cfg.iMaxIter // 10, 0.5, True)
else:
    lrG = cfg.fLrIniG
...
gen_optimizer = tf.train.AdamOptimizer(
    learning_rate=lrG, beta1=cfg.fBeta1, beta2=cfg.fBeta2, epsilon=cfg.fEpsilon)

gen_gradient_values = gen_optimizer.compute_gradients(
    gen_total_loss, var_list=gen_vars)
gen_optimize_ops = gen_optimizer.apply_gradients(
    gen_gradient_values, global_step=global_step)
...
dis_optimizer = tf.train.AdamOptimizer(
    learning_rate=lrD, beta1=cfg.fBeta1, beta2=cfg.fBeta2, epsilon=cfg.fEpsilon)
...
if cfg.bLoadPre:
    if cfg.sPreTag == "":
        assert False
    load_dis_vars = [v for v in tf.trainable_variables(
    ) if 'discriminator' in v.name and 'linear' not in v.name]
    #load_dis_vars = [
    #    v for v in load_dis_vars if 'discriminator/' in v.name]
    for var in load_dis_vars:
        print(var.name)
    loader_dis = tf.train.Saver(load_dis_vars)

    print([v.name for v in load_dis_vars])
    ini_model(sess)
    try:
        if load_model(loader_dis, sess, '../result/' + cfg.sPreTag + '/checkpoint'):
            logger.log(" [*] Load Pretrained SUCCESS")
            # logger.load()
            logger.tick(iter)
            logger.log('\n\n')
            logger.flush()
...
fixed_noise = tf.constant(np.random.normal(
    size=(100, cfg.iDimsZ)).astype('float32'))
fixed_noise_gen = generator(100, fixed_noise)

logger.log("Generator Total Parameter Count: {}".format(
    locale.format("%d", param_count(gen_gradient_values), grouping=True)))
...
while iter <= cfg.iMaxIter:
    ...
    for id in range(cfg.iTrainD):
        ...
        _, _dis_total_loss, _dis_gan_loss, _lr, _real_logits, _fake_logits = sess.run(
            [dis_optimize_ops, dis_total_loss, dis_gan_loss,
                lrG, real_logits, fake_logits],
    ...
    for ig in range(cfg.iTrainG):
        _, _gen_total_loss, _gen_gan_loss, _lr, _fake_logits = sess.run(
            [gen_optimize_ops, gen_total_loss, gen_gan_loss, lrG, fake_logits])
    ...
    if (iter + 1) % 1000 == 0:
        #icp_score, am_score, fid = get_score(gen_n_images(1000))
        ...
        if cfg.sDataSet == 'toy':
            ...
            save_images(_fixed_noise_gen.transpose(0, 2, 3, 1), [
                        10, 10], '{}/train_{:02d}_{:04d}.png'.format(sSampleDir, iter // 10000, iter % 10000))
    ...
    # # if time.time() - last_log_time > 6*1:
