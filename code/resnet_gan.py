...
tf.app.flags.DEFINE_string("sResultTag", "gan_v0", "your tag for each test case")
...
tf.app.flags.DEFINE_integer("iMaxIter", 500000, "")
tf.app.flags.DEFINE_integer("iBatchSize", 1024, "")
...
def residual_block_generator(input, filters, ksize, stride, name=None):
    ...
    identity = deconv2d(h0, filters, ksize=ksize, stride=stride, name=name+'identity')
...
def generator_dcgan(num_sample, z=None):
    ...
    with tf.variable_scope('generator', tf.AUTO_REUSE):
        ...
        h0 = residual_block_generator(h0, iFilterDimsG * 8,ksize=3,stride=2,name='generator_block1_4x4')
        h0 = residual_block_generator(h0, iFilterDimsG * 4,ksize=3,stride=2,name='generator_block2_8x8')
        h0 = residual_block_generator(h0, iFilterDimsG * 2,ksize=3,stride=2,name='generator_block3_16x16')
...
def get_score(samples):
    ...
    if ref_icp_activations is None:
        ...
        ref_icp_preds, ref_icp_activations = icp_model.get_preds(data_X.transpose(0, 2, 3, 1))
...
if cfg.oDecay == 'linear':
    ...
    lrD = tf.train.exponential_decay(cfg.fLrIniD, global_step, cfg.iMaxIter // 10, 0.5, True)
else:
    lrD = cfg.fLrIniD
...
if cfg.bLoadPre:
    if cfg.sPreTag == "":
        assert False
    load_dis_vars = [v for v in tf.trainable_variables() if 'discriminator' in v.name and 'linear' not in v.name]
    load_dis_vars = [v for v in load_dis_vars if 'discriminator/conv32/' in v.name]
...
while iter <= cfg.iMaxIter:
    ...
    for id in range(cfg.iTrainD):
        ...
        _, _dis_total_loss, _dis_gan_loss, _lr, _real_logits, _fake_logits = sess.run(
            [dis_optimize_ops, dis_total_loss, dis_gan_loss, lrG, real_logits, fake_logits],
