...
def load_imagenet_32(chunk_num):
    import os,sys
    def unpickle(file):
        fo = open(file, 'rb')
        if sys.version_info[0] >= 3:
    ...
    # data_dir = SOURCE_DIR + 'dataset/imagenet/'

    # data_X = []
    # data_Y = []

    # #try:
    # trfilenames = [os.path.join(data_dir, 'train_data_batch_%d' % i) for i in range(1, 6)]
    # for files in trfilenames:
    #     dict = unpickle(files)
    #     data_X.append(np.reshape(np.array(dict["data"]),(-1,3,64,64))[:,:,::2,::2])
    #     data_Y.append(dict["labels"])


    # data_X = np.concatenate(data_X,0)
    # data_Y = np.concatenate(data_Y, 0)
    # #data_X = np.reshape(data_X, [-1, 64, 64,3])
    ...
    def create_dataset_from_binary(data_dir):
        
        for i in range(5):
            data_X = []
            data_Y = []
            print("Creating chunk %d   ."%i)
            trfilenames = [os.path.join(data_dir, 'train_data_batch_%d' % j) for j in range(i*2+1,i*2+3)]
            for files in trfilenames:
                dict = unpickle(files)
                data_X.append(np.reshape(np.array(dict["data"],dtype=np.float32),(-1,3,64,64))[:,:,::2,::2])
                data_Y.append(dict["labels"])
            ...
            #
            data_Y = np.reshape(data_Y, [len(data_Y)]).astype(np.int32)
            data_X.tofile(data_dir +'train_32_X_chunk_%d' %i)
            data_Y.tofile(data_dir +'train_32_Y_chunk_%d' %i)
            #np.save(data_dir +'train_32_X_chunk_%d' %i,data_X)
            #np.save(data_dir + 'train_32_Y_chunk_%d' %i,data_Y)
    
    data_dir = data_dir = SOURCE_DIR + 'dataset/imagenet/'
    for i in range(5):
        if not os.path.isfile(data_dir+ 'train_32_X_chunk_%d' %i):
            print("Creating Dataset from batch files")
            create_dataset_from_binary(data_dir)
        if not os.path.isfile(data_dir+ 'train_32_Y_chunk_%d' %i):
            print("Creating Dataset from batch files")
            create_dataset_from_binary(data_dir)
    print("Loading Dataet chunks Requested")
    #return np.load(data_dir + 'train_32_X_chunk_%d.npy' %chunk_num), np.load(data_dir + 'train_32_Y_chunk_%d.npy' %chunk_num)
    return np.reshape(np.fromfile(data_dir + 'train_32_X_chunk_%d' %chunk_num, dtype=np.float32),(-1,3,32,32)), np.fromfile(data_dir + 'train_32_Y_chunk_%d' %chunk_num, dtype=np.int32)
...
def load_imagenet():
    ...
    data_dir = SOURCE_DIR + 'dataset/imagenet/'
    if not os.path.exists(data_dir):
        print("Error: Dataset not present")
        return
    ...
    if(1 == 1):
        ...
        data_X = (np.concatenate(data_X, 0))
        data_X = (np.reshape(data_X, [-1, 3, 64, 64])).astype(np.float32)
        #data_X = np.transpose(data_X, [0, 2, 3, 1])
        data_X = (data_X - 127.5) / 128.0
        data_Y = np.concatenate(data_Y, 0)-1
        data_Y = np.reshape(data_Y, [len(data_Y)]).astype(np.int32)
        ...
        ##test_Y = np.concatenate(test_Y, 0)
        ##test_Y = np.reshape(test_Y, [len(test_Y)]).astype(np.int32)
        test_X = data_X[0:-1000]
        test_Y = data_Y[0:-1000]
        data_Y = data_Y[0:len(data_Y)-1000]
